...
if "initialized" not in st.session_state:
    ...
    st.session_state.logs = ["🎮 歡迎來到【華爾街見習生】網頁版！", "準備好開始你的投資之旅了嗎？請在下方進行操作！"]
...
if st.session_state.game_over:
    ...
    def handle_action(action_type):
        ...
        st.session_state.logs = action_log + event_logs + advisor_logs + st.session_state.logs
...
# =====================
st.divider()
st.subheader("📝 遊戲紀錄與導師回饋")

with st.container(border=True):
    for log in st.session_state.logs:
        if log.startswith("❌") or log.startswith("💥") or log.startswith("⚠️"):
            st.error(log)
